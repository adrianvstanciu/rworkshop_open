...
<!--- #RS# Since we point out that there is only one man, I have added n() to summarise() --->
...
(3) rename `ep01` into `germ_econ_now` and `ep03` into `germ_econ_future`
...
```{r}
#RS# I have removed the answers?
# Also, summarise is the next lesson.
# Instead, I would say people should save the filtered data and then calculate mean and sd.
```
