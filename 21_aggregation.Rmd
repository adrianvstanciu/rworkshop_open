...
`R` basically does the following - but, again, in the background:

<!--- #RS#
Ich bin mir unsicher, ob das hier hilft.
group_by() macht etwas deutlich anderes als split()
split teilt den data.frame und packt die Teile in eine Liste.
group_by() fügt dem data.frame die Klasse "grouped_df" hinzu und speichert die Gruppierungsinformationen im speziellen Attribut $groups

Ich würde alles zu split() hier weglassen. Ich habe split() mit bei den purrr Beispielen.
Da passt es gut, weil man mit map() gut mit Listen umgehen kann.
--->

See below the difference:

```{r}
# to illustrate what the group_by() function does in the background
split(df_allbus, df_allbus$sex_fct)
```

As you see, two data frames are formed from the original data frame. Now let us go back to actually working with data, and aggregating after having grouped the data. 

<!--- #RS# Vorschlag:
Was group_by() macht kann man vielleicht so zeigen: --->
...
<!--- #RS#
across() kommt doch erst am dritten Tag?
Ich würde hier wirklich noch manuell arbeiten 
Vor allem würde ich nicht gleich so einsteigen.
--->
...
Listing variables and function manually is a hassle. Often we want to apply a set of aggreation functions to a set of variables in one go. That can be easily done with across(), which we will discuss tomorrow!
