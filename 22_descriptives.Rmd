...
glimpse(df_allbus)
```
If we look at the factor versions "_fct", we can alrady guess that some variables are more categorical (e.g., `work_fct`), while others are metric (e.g., `age`), or (pseudo-)metric (e.g., `ep01_fct`). Some variables remain cryptic, however. Look at `pa01_fct`. What are those letters?
...
<!--- #RS# 
I hope you don't mind, but I think explaining that here goes to far.
Especially since plotting is complex enough.

If you want to keep it in, I would prefer a rewritten version in tidyverse style with map() instead of lapply().


## Outlook: Automation with `purrr::map()`

Repetetive code is often easily automated in R. 
Imagine we want frequency tables for all variables.

That can be easily done with `map()`, which we will discuss tomorrow. In short, the family of map functions from `purrr::` applies a function to each element in a vector or dataframe or list. 

Here, we enrich the allbus_labels dataframe, by adding frequency tables!

```{r}

# Function that takes a vector and a name and creates a frequency table
generate_freq_tab <- function(variable){
  tibble(variable) %>% 
    group_by(responses = variable) %>% 
    summarise(abs_freq= n()) %>% 
  mutate(
    rel_freq=abs_freq*100/sum(abs_freq), 
    cum_rel_freq = 100*cumsum(rel_freq/sum(rel_freq))
  )
}

# Apply the function to all variables listed in allbus_labels
# Note how we use allbus_labels for the variable names, but then draw data from df_allbus!

labels_and_tables <- allbus_labels %>% 
  select(-value_labels) %>% 
  mutate(
    frequency_tabs = map(paste0(var_name, "_fct"),
                          ~ generate_freq_tab(df_allbus[[.x]]))
  )

# If we want to access the frequency table, we can simply unnest() it.
# This creates a long-format data frame, ideal for further analyses

# This is what it looks like for the variable "work":

labels_and_tables %>% 
  filter(var_name == "work") %>% 
  unnest(frequency_tabs)
...
<!--- I removed formal tests because this is part of the next lesson 23--->
...
<!--- vorschlag für einfachere Übungen --->
# Exercises
