...
summary(allbus_df$sex)

summary(allbus_labels)
```
...
```{r}
mean(allbus_df$pa01_fct)

mean(allbus_df_spss$pa01)

mean(allbus_df$pa01)

# we need the na.rm = T argument to instruct R how to handle the missing information
# without that argument R would not be able to calculate the mean or median
mean(allbus_df$pa01, na.rm = T)
median(allbus_df$pa01, na.rm = T)
```
